...
```{r, eval = FALSE}
# Set the working directory to the folder where your project files are located
setwd("~/R/Cyclistic")       
```      
...
By implementing these initiatives together, we create a seamless cycle that builds awareness, drives conversions through personalized offers, and provides an affordable entry point for casual riders. 

This approach not only maximizes membership growth but also ensures long-term engagement and retention, paving the way for future upgrades to full membership.
